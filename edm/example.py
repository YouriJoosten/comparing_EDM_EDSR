...
def load_custom_image(image):
    ...
    transform = transforms.Compose([
        transforms.Resize((32, 32)),  # Resize to 32x32 (same as CIFAR-10 images)
        transforms.ToTensor(),
        transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])  # Normalize for the model
    ])
    return transform(image).unsqueeze(0)  # Add batch dimension


def test_upsample(image, model_path, output):
    """Load an image, process it with EDM, and save the result."""

    # Define where the model should be saved locally
    complete_path = model_path+"/edm-cifar10-32x32-cond-vp.pkl"
    # Load the model
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f'Loading network from {complete_path}')
    with open(complete_path, 'rb') as f:
        net = torch.load(f, map_location=device)['ema'].to(device)  # FIXED ERROR

    # Load the image
    custom_image = image.to(device)

    # Run EDM processing
    with torch.no_grad():
        output_tensor = net(custom_image)

    # Convert to image and save
    output_image = (output_tensor * 127.5 + 128).clip(0, 255).to(torch.uint8)
    output_image = output_image.cpu().numpy().transpose(1, 2, 0)
    PIL.Image.fromarray(output_image, 'RGB').save(output)
    print(f"Upsampled image saved to {output}")
